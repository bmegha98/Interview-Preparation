...
Return true if Player 1 can win the game. If the scores of both players are equal, then player 1 is still the winner, and you should also return true. You may 
...
class Solution {
    public boolean PredictTheWinner(int[] nums) {
        ...
        if(n <= 2)
            return true;
        int player1Score = optimalScore(nums,0,n-1);
        int sum = 0;
        for(int x : nums)
            sum += x;
    ...
    private int optimalScore(int[] nums, int i, int j) {
        if(i > j)
            return 0;
        if(i == j)
            return nums[i];
        ...
        int first = nums[i]+Math.min(optimalScore(nums,i+2,j),
                                    optimalScore(nums,i+1,j-1));
        int last = nums[j]+Math.min(optimalScore(nums,i+1,j-1),
                                   optimalScore(nums,i,j-2));
