/*
There are given N ropes of different lengths, we need to connect these ropes into one rope. The cost to connect two ropes is equal to sum of their lengths. The
...
class Solution
{
    ...
    long minCost(long arr[], int n) 
    {
        ...
        if(n == 1)
            return 0;
        Queue<Long> aux = new PriorityQueue<>();
        for(long ele : arr)
            aux.add(ele);
        
        long totalCost = 0;
        while(aux.size() > 1) {
            long rope1 = aux.poll(),rope2 = aux.poll();
            long combined = rope1+rope2;
            totalCost += combined;
            aux.add(combined);
        }
        return totalCost;
