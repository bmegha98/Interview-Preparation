/*
Given an array arr[] containing positive elements. ‘A’ and ‘B’ are two numbers defining a range. The task is to check if the array contains all elements 
in the given range.
...
check_elements() that takes array arr, integer N, integer A, and integer B  as parameters and returns the boolean True if array elements contain all
elements in the given range else boolean False.
    ...
    boolean check_elements(long arr[], int n, int A, int B)
    {
        for(int i=0;i<n;i++)
            if(Math.abs(arr[i]) >= A && Math.abs(arr[i]) <= B) {
                int ind = (int)(Math.abs(arr[i])-A);
                if(ind < n && arr[ind] > 0)
                    arr[ind] *= -1;
            } 
        
        int count = 0, target = B-A+1;
