...
Output: false
Explanation: The first three characters "app" match, and the second string is shorter (in size.) According to lexicographical rules "apple" > "app", because
'l' > '∅', where '∅' is defined as the blank character which is less than any other character (More info).
*/
    ...
    public boolean isAlienSorted(String[] words, String order) {
        ...
        for(int i = 0;i<n-1;i++) {
            ...
            while(j < s.length() && j < t.length()) {
                ...
                if(orderAux[x-'a'] > orderAux[y-'a'])
                    return false;
                break;
            }
            
            if(j == Math.min(s.length(),t.length()) && s.length() > t.length())
                return false;
        }
        return true;
    }
