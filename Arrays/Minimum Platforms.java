...
class TrainTime {
    private int value;
    private char type;
    public TrainTime(int val, char t) {
        value = val;
        type = t;
    }
    public int getValue() {
...
class Solution
{
    ...
    {
        // add your code here
        Queue<TrainTime> trains = new PriorityQueue<>(1, (a,b)-> {
        ...
        for(int i=0;i<n;i++) {
            trains.add(new TrainTime(arr[i],'a'));
            trains.add(new TrainTime(dep[i],'d'));
        }
        
        int count = 0, minPlatforms = 0;
        while(!trains.isEmpty()) {
            TrainTime t = trains.poll();
            if(t.getType() == 'a')
                count++;
            else
                count--;
